...
public class ExamService {
	...
	@Transactional(readOnly=false)
	public Exam generatePaper(Teacher t,List<Integer> list,List<Integer> list1,Double averageScore){
		List<Question> questions=examDao.findQuestionById(list);
		List<QuestionType> types=questionDao.findAllQuestionType();
		Exam e=examDao.newExam(t,averageScore);
	...
	public Exam newExam(Teacher t,Double averageScore){
		return examDao.newExam(t,averageScore);
	}
	
	@Transactional(readOnly=false)
	public Exam hand_NewExam(Teacher t){
		return examDao.hand_NewExam(t);
	}
	...
	@Transactional(readOnly=false)
	public void addPointToExam(Exam exam, List<Integer> intList) {
		boolean b=false;
		...
		System.out.println("");
		if(b==true){
			examDao.insertPointValueToExam(exam,intList);
		}else{
