...
public class QuestionDao {
	...
	public Page findQuestionByChapterAndTypeAndLevel(Integer chapter_id,Integer type_id,Integer level_id,int currentPage, int pageSize){
		...
		if(chapter_id==0&&type_id==0&&level_id==0){
			query=session.createQuery("from Question where repeats=0");
		}else if(chapter_id==0&&type_id==0){
			query=session.createQuery("from Question where level_id=? and repeats=0");
			query.setInteger(0, level_id);
		}else if(type_id==0&&level_id==0){
			query=session.createQuery("from Question where chapter_id=? and repeats=0");
			query.setInteger(0, chapter_id);
		}else if(chapter_id==0&&level_id==0){
			query=session.createQuery("from Question where type_id=? and repeats=0");
			query.setInteger(0, type_id);
		}else if(chapter_id==0){
			query=session.createQuery("from Question where type_id=? and level_id=? and repeats=0");
			query.setInteger(0, type_id);
			query.setInteger(1, level_id);
		}else if(type_id==0){
			query=session.createQuery("from Question where chapter_id=? and level_id=? and repeats=0");
			query.setInteger(0, chapter_id);
			query.setInteger(1, level_id);
		}else if(level_id==0){
			query=session.createQuery("from Question where chapter_id=? and type_id=? and repeats=0");
			query.setInteger(0, chapter_id);
			query.setInteger(1, type_id);
		}else{
			query=session.createQuery("from Question where chapter_id=? and type_id=? and level_id=? and repeats=0");
			query.setInteger(0, chapter_id);
			query.setInteger(1, type_id);
	...
	//增加没有选项的题
		...
		public float searchRepeat(String content,int chapter){
			...
			for(int m=0;m<list.size();m++){
				...
				while(quescontent.contains("<")){
					String sub=quescontent.substring(quescontent.indexOf("<"), quescontent.indexOf(">")+1);
					quescontent=quescontent.replace(sub,"");
				}
		...
		public float searchRepeat(String content,String A,String B,String C,String D,int chapter){
			...
			for(int m=0;m<list.size();m++){
				...
				while(quesA.contains("<")){
						String sub=quesA.substring(quesA.indexOf("<"), quesA.indexOf(">")+1);
						quesA=quesA.replace(sub,"");
				}
				while(quesB.contains("<")){
						String sub=quesB.substring(quesB.indexOf("<"), quesB.indexOf(">")+1);
						quesB=quesB.replace(sub,"");
				}
				while(quesC.contains("<")){
						String sub=quesC.substring(quesC.indexOf("<"), quesC.indexOf(">")+1);
						quesC=quesC.replace(sub,"");
				}
				while(quesD.contains("<")){
						String sub=quesD.substring(quesD.indexOf("<"), quesD.indexOf(">")+1);
						quesD=quesD.replace(sub,"");
				}
	...
	public int updateQuestion(int id,String content,int chapter,int type,int level,String answer,String t_description,
	...
	public List<Question> allCanChangeQuestion(Integer e_id,Integer q_id){
		...
		Query query=session.createQuery("from Question where type_id=? and level_id=? and chapter_id=? and repeats=0");
