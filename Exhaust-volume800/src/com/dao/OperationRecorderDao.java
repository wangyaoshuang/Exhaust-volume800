...
public class OperationRecorderDao {
	...
	public Page FindAlllist(int currentPage,int pageSize){
		...
		Query query=session.createQuery("from Recorder order by id desc");
	...
	public Page findTeacherRecorder(int currentPage,int pageSize,Integer t_id){
		Session session = sessionFactory.getCurrentSession();
		Query query = session.createQuery("from Recorder where t_id=? order by id desc");
	...
	public Page findRecorderByOperationType(int currentPage,int pageSize,String operationName){
		...
		if(operationName.equals(null) || operationName.equals("")){
			...
			//先查询出操作类型的id
			Query query = session.createQuery("from Operation where name=? order by id desc");
			...
			//根据id查询记录
			Query query_list = session.createQuery("from Recorder where o_id = ? order by id desc");
	...
	public Page findRecorderByTeacherName(int currentPage,int pageSize,String teacherName){
		Session session = sessionFactory.getCurrentSession();
		Query query = session.createQuery("from Teacher where name like ? order by id desc");
