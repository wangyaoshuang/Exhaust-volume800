...
<script type="text/javascript">
	function check(){
		var hiredate=document.getElementsByName("hiredate")[0];
	...
	$(document)
			.ready(
					function() {
						...
						//验证入职时间
						$("input[name='hiredate']").mouseout(function() {
							$(this).css("border", "1px solid #dddddd");
						})
						$("input[name='hiredate']").mouseover(function() {
							$(this).css("border", "1px solid #80b6ea");
						})
						$("input[name='hiredate']").blur(function() {
...
<body>
	...
	<div class="pd-20">
		...
		<div class="everyone">
			<span class="red">&nbsp;&nbsp;*</span> <span>入职日期：</span> <input
				type="date" value="${teac.hiredate }" name="hiredate" id="hiredate"/> <span
		...
		<c:if test="${action=='edit' }">
			<div>
				<input type="radio" name="xuanze" value="shanchu" checked="cheched"/>保留课程:<br />
				<c:forEach items="${teac.courses}" var="courses2">
						&nbsp;&nbsp;&nbsp;<input align="center" type="checkbox" checked="checked"
				...
				<br /> <input type="button" value="修改" onclick="check()"/>
			</div>
		</c:if>
		<c:if test="${action!='edit'}">
				添加课程:<br />
