...
<section class="vbox">
      	...
      	<section id="content">
        	<section class="vbox">
          		<section class="scrollable padder">
            		<div class="m-b-md">
              			<h3 class="m-b-none"><a href="${ctx }/question/preview">工作台</a></h3>
              			<small>Welcome back, ${teacher.name }</small>
            		</div>
            		<section class="panel panel-default">
	<table class="table table-hover">
		<tr>
		<form method="post" action="${ctx }/question/allQuestion">
			...
			<c:forEach items="${page.list }" var="q">
				...
				<tr height="60">
					...
					<td  width="290px">
					<c:if test="${fn:length(q.content) > 20 }">
						${fn:substring(q.content,0,20)}......
					</c:if>
					<c:if test="${fn:length(q.content)  <= 20 }">
						${q.content}
					</c:if></td>
					...
					<c:if test="${b==0 }">
						...
						<input type="button" name="select" value="查看详情"/ onclick='window.open("${ctx}/question/QuestionDetails?id="+${q.id})'>
		...
		<c:set var="url" value="${ctx }/question/allQuestion?course_id=${course_id }&chapter_id=${chapter_id}&type_id=${type_id }&level_id=${level_id}"/>
		<tr>
	...
	<table cellSpacing=2 cellPadding=2 id="summarize"  width="180px">
		<tr>
			<td>试卷中所选题型数量列表</td><td colspan="3" ></td>
		</tr>
	...
	<div id="submit1"><input id="submit2" type="button" value="生成试卷" onclick="window.location.href='${ctx}/exam/toAddPointValue'"></div>
	</section>
        		</section>
        	</section>
        </section>
