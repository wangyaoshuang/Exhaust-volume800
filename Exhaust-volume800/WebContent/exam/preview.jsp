...
$(function(){
        ...
        $(".moveup").live("click",function(){
                    ...
                    var sub_id=exam_need.substring(0,exam_need.indexOf(","));
		        	var e_id=${exam.id};
		        	$.ajax({
		        		type: "POST",
		        		url: "${ctx}/exam/moveup",
		        	...
		        	exam_need=exam_need.substring(exam_need.indexOf(",")+1,-1);
		        	var q_id=onthis.attr("id");
		        	sub_id=parseInt(sub_id)-1;
		        	var ns=onthis.next();
		        	var exam_need_ns=ns.attr("exam_need");
		        	exam_need_ns=exam_need_ns.substring(exam_need_ns.indexOf(",")+1,-1);
		        	var q_id_ns=onthis.next().attr("id");
		        	onthis.attr('exam_need',sub_id+','+q_id);
		        	ns.attr('exam_need',sub_id+1+','+q_id_ns);
        ...
        $(".movedn").live("click",function(){
		        	...
		        	var exam_need_ns=ns.attr("exam_need");
		        	exam_need_ns=exam_need_ns.substring(exam_need_ns.indexOf(",")+1,-1);
		        	var q_id_ns=onthis.prev().attr("id");
...
<body  onload = "BodyOnLoad();" >
	...
	<div id="xz">
		...
		<form action="${ctx }/test/Excel" method="post">
			<input type="hidden" value="${exam.id}" name="ed"/> 
			<input type="submit" value="下载命题明细表" id="mingxi"/>
		</form>
		<form action="${ctx }/test/Answer" method="post">
			<input type="submit" value="下载答案" id="shijuan"/>
		</form>
		<form action="${ctx }/test/topic" method="post">
			<input type="submit" value="下载审题登记表" id="shijuan"/>
		</form>
	<input id="submit2" type="button" value="保存试卷" onclick="window.location.href='${ctx}/exam/listExam'">
	</div>
